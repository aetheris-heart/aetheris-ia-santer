...
const AetherisHeartUltra: React.FC<AetherisHeartUltraProps> = ({
  ...
  useFrame(({ clock }) => {
    ...
    if (heartRef.current) heartRef.current.scale.set(beat, beat, beat);

    // Mouvement fluide des particules sanguines
    if (bloodParticles.current) {
      bloodParticles.current.rotation.y += 0.0015;
      const positions = (bloodParticles.current.geometry as THREE.BufferGeometry).attributes
        .position as THREE.BufferAttribute;
      for (let i = 0; i < positions.count; i++) {
        positions.array[i * 3 + 1] += Math.sin(t + i) * 0.0005;
      }
